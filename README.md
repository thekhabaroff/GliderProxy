...
Меню **«Статистика»** сначала показывает сводку по каждому пользователю:
...
В пункте **«Пользователи»** можно выбрать конкретного пользователя и открыть детальную статистику по его порту:

```text
НАПРАВЛЕНИЕ      TCP          UDP          ВСЕГО
Входящий         1.20 GiB     50.00 MiB    1.25 GiB
Исходящий        300.00 MiB   20.10 MiB    320.10 MiB
```

- **Входящий TCP/UDP** — байты, пришедшие на порт пользователя (`INPUT --dport`)
- **Исходящий TCP/UDP** — байты, отправленные с порта пользователя (`OUTPUT --sport`)
